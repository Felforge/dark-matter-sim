...
class BarnesHut {
    ...
    private:
        ...
        void createChildren() {
            ...
            // Prefetch d_counts to the GPU.
            CUDA_CHECK(cudaMemPrefetchAsync(dOctantCounts, 8 * sizeof(int), device, 0));
...
__global__ void createChildrenKernel(Particle* inpBodies, int inpNumBodies, Particle* octantLists[8], int* octantCounts, double mid[3]) {
    ...
    // Diagnostic check: ensure octant is valid
    if (octant < 0 || octant >= 8) {
        printf("Invalid octant %d computed for idx %d\n", octant, idx);
        return;
    }
    
    // Print the address (only a few threads to avoid flooding)
    if (idx % 7 == 0) {
        printf("dOctantCounts address: %p\n", (void*)octantCounts);
    }

    // Get index for this particle in the octant list
    int insertIdx = atomicAdd(&octantCounts[octant], 1);
    // int insertIdx = octantCounts[octant] + 1;
    // octantCounts[octant] = insertIdx;
...
// LOOK AT TROUBLESHOOTING STUFF THAT WAS ALREADY ADDED IN LATER
